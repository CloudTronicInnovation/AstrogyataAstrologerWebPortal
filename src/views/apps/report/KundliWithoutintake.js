...
const KundliWithoutintake = (props) => {
  ...
  useEffect(() => {
    if (localStorage.getItem("kundliData")) fetchPanchangDetails();
    setBasicDetails(JSON.parse(localStorage.getItem("kundliData")));
  ...
  const fetchPanchangDetails = () => {
    const { dateofbirth, birthplace, hours, minutes } = JSON.parse(
      localStorage.getItem("kundliData")
    );
    const { day, month, year } = getDayMonthYear(dateofbirth);
    const latitude = birthplace?.city?.latitude || birthplace?.state?.latitude 
    const longitude = birthplace?.city?.longitude || birthplace?.state?.longitude
                    ...
                    >
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            {getAMPM(
                              basicDetails?.hours?.split(":")[0],
                              basicDetails?.minutes?.split(":")[0],
                        ...
                        <tr>
                          ...
                          <td>{basicDetails && basicDetails?.birthplace?.city.name}</td>
                        </tr>
                        {/* <tr>
                    <th>Latitude</th>
                    <td>
...
export default React.memo(KundliWithoutintake);
